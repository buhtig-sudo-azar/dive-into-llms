...
export function ApiTokenInput() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // If we get a response (not a 401/403), token is valid
    ...
    <div className="space-y-2">
      ...
      {!isExpanded && hasToken && (
        <div className="flex items-center gap-2">
          ...
          <HelpTooltip content="Удалить токен — вернуться к общему токену платформы" side="top">
            <Button
              variant="ghost"
              size="sm"
      ...
      {isExpanded && (
        <div className="space-y-2">
          <div className="flex gap-2">
            <HelpTooltip
              content="Вставьте ваш OpenRouter API-токен. Получить бесплатно: openrouter.ai/keys — токен хранится только в вашем браузере и никогда не отправляется на наш сервер."
              side="bottom"
            >
              <Input
                ref={inputRef}
