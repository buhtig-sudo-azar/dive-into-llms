...
export const contextEngineering: TopicCategory = {
  ...
  return [
    ...
    { role: 'user', content: query }
      ...
      commonMistakes: [
        {
          mistake: 'Кладить всю информацию без приоритизации',
    ...
    },
    {
      ...
      theory: {
        terms: [
          { term: 'Short-term Memory', definition: 'Контекст текущего диалога, хранящийся в контекстном окне. Ограничен размером окна и теряется при новом диалоге.' },
          { term: 'Long-term Memory', definition: 'Внешнее хранилище (БД, векторная БД), сохраняющее информацию между сессиями. Извлекается через RAG при необходимости.' },
          { term: 'Episodic Memory', definition: 'Память о конкретных событиях и взаимодействиях. Хранит «что произошло когда».' },
          { term: 'Semantic Memory', definition: 'Обобщённые знания и факты, извлечённые из взаимодействий. Хранит «что является истинным».' },
        ],
        terms: [],
        principles: 'Память LLM строится по иерархии: рабочая память (контекстное окно) → краткосрочная (история сессии) → долгосрочная (векторная БД). Рабочая память ограничена, но мгновенна. Долгосрочная безгранична, но требует поиска и извлечения. Эффективные системы памяти комбинируют оба уровня: важная информация немедленно доступна в контексте, а остальное извлекается по запросу через RAG.',
        architecture: 'Типичная система памяти включает: хранилище (векторная БД + реляционная БД), индексатор (преобразует взаимодействия в embeddings), извлекатель (находит релевантные воспоминания), и интегратор (вставляет воспоминания в контекст). При каждом взаимодействии система решает, что сохранить, что извлечь и как организовать в контексте.',
  ...
  async getRelevantContext(query: string, limit = 5) {
    ...
    },
    {
      ...
      theory: {
        terms: [
          { term: 'Context Truncation', definition: 'Простое удаление старых или нерелевантных частей контекста. Быстро, но может потерять важные данные.' },
          { term: 'Context Distillation', definition: 'Сжатие контекста через LLM: модель перефразирует длинный контекст в краткую сводку, сохраняя ключевые факты.' },
          { term: 'Selective Context', definition: 'Метод оценки важности каждого фрагмента контекста и удаления наименее значимых.' },
          { term: 'LLMLingua', definition: 'Инструмент от Microsoft для сжатия промптов путём удаления малозначимых токенов с минимальной потерей качества.' },
        ],
        terms: [],
        principles: 'Сжатие контекста работает по принципу информационной ценности: каждый фрагмент оценивается по вкладу в качество ответа. Низкоинформативные фрагменты удаляются или сжимаются. Высокоинформативные сохраняются целиком. Ключевая метрика — соотношение сжатия к потере качества: хорошее сжатие уменьшает объём на 50-70% с потерей качества менее 5%.',
        architecture: 'Система сжатия включает: оценщик важности (определяет ценность каждого фрагмента), компрессор (удаляет или сжимает маловажные части), валидатор (проверяет, что критическая информация сохранена). Оценка важности может основываться на注意力 модели, векторном сходстве с запросом или эвристиках (давность, длина, позиция).',
        connections: 'Сжатие связано с Summarization (частный случай сжатия), Memory (архивация старых данных), RAG (выборка наиболее релевантных фрагментов) и управлением контекстом в целом.',
      },
      theory: {
        terms: [
  ...
  return [
    ...
    },
    {
      ...
      theory: {
        terms: [
          { term: 'Extractive Summarization', definition: 'Выбор наиболее важных предложений из исходного текста без изменения. Быстро, но может быть несвязной.' },
          { term: 'Abstractive Summarization', definition: 'Генерация нового краткого текста, передающего смысл оригинала. LLM-подход, даёт более связные результаты.' },
          { term: 'Map-Reduce Summarization', definition: 'Метод суммаризации очень длинных текстов: текст разбивается на части, каждая суммаризируется отдельно, затем сводки объединяются.' },
          { term: 'Recursive Summarization', definition: 'Итеративная суммаризация: текст суммаризируется, затем результат суммаризируется снова, пока не достигнет нужного объёма.' },
        ],
        terms: [],
        principles: 'LLM-суммаризация работает лучше всего, когда промпт чётко определяет: что важно (критерии), для кого (аудитория), какой длины (целевой объём) и в каком формате (стиль, структура). Качество суммаризации сильно зависит от этих параметров. Для очень длинных текстов используется Map-Reduce: текст бьётся на части, каждая суммаризируется отдельно, затем промежуточные сводки объединяются в финальную.',
        architecture: 'Архитектура суммаризации зависит от длины текста: короткий текст (< контекстное окно) — прямая суммаризация одним запросом. Средний текст — суммаризация с чанками. Длинный текст — Map-Reduce или рекурсивная суммаризация. Критически важна цепочка: исходный текст → чанки → промпт суммаризации → LLM → результат → (опционально) повторная суммаризация.',
        connections: 'Суммаризация — ключевой инструмент для Compression (сжатие контекста), Memory (архивация старых диалогов), RAG (суммаризация извлечённых документов) и Production AI (обработка больших потоков данных).',
      },
      theory: {
        terms: [
