  ...
  subtopics: [
    {
      ...
      introduction: {
        ...
        problem: 'Zero-shot решает проблему быстрого старта: вам не нужно готовить примеры или обучать модель. Достаточно чётко сформулировать задачу, и модель справится, опираясь на свои обширные знания. Это демо кратизирует доступ к AI-возможностям.',
      },
      theory: {
        terms: [
          ...
          { term: 'Temperature', definition: 'Параметр генерации, контролирующий разнообразие ответов. При zero-shot часто ставят низкую температуру (0.0-0.3) для предсказуемости.' },
        ],
        terms: [],
    ...
    },
    {
      ...
      theory: {
        terms: [
          ...
          { term: 'Auto-CoT', definition: 'Автоматическая генерация цепочек рассуждений для few-shot примеров, устраняющая необходимость ручного написания.' },
...
// Ответ: 7 яблок`,
    ...
    },
    {
      ...
      categorySlug: 'prompt-engineering',
      introduction: {
        what: 'ReAct (Reasoning + Acting) — это парадигма, объединяющая рассуждения (Chain of Thought) с выполнением действий (Tool Use). Модель чередует шаги «думать» и «действовать»: она рассуждает о задаче, решает какое действие предпринять, выполняет его через инструмент, наблюдать результат и продолжает цикл.',
        ...
        where: 'ReAct применяется в AI-агентах, системах问答 с доступом к инструментам, автономных ассистентах, аналитических системах и любом сценарии, где LLM должен взаимодействовать с внешним миром.',
...
async function reactAgent(question: string) {
  ...
  while (true) {
    ...
    },
    {
      ...
      theory: {
        terms: [
          { term: 'JSON Mode', definition: 'Режим API, гарантирующий, что ответ модели будет валидным JSON. Доступен в OpenAI, Anthropic и других API.' },
          { term: 'Function Calling', definition: 'Механизм, при котором модель генерирует структурированный вызов функции с параметрами вместо текстового ответа.' },
        ...
        terms: [],
        principles: 'Structured Output работает двумя путями: на уровне промпта (инструкция + примеры формата) и на уровне API (JSON Mode, Function Calling). Промпт-подход гибче, но менее надёжен. API-подход гарантирует валидный формат, но ограничивает доступные модели. Лучший результат — комбинация обоих: чёткий промпт с примером + JSON Mode API. Критически важно указывать в промпте, что модель НЕ должна добавлять текст до или после JSON.',
        architecture: 'При использовании Function Calling модель получает описание функций (имя, параметры, типы) и вместо генерации текстового ответа выбирает функцию и форматирует аргументы в JSON. Это реализовано через специальные токены в обучении модели. JSON Mode работает иначе: модель генерирует текст как обычно, но логит-биас принуждает к генерации только валидных JSON-токенов. Оба подхода гарантируют парсибельный результат.',
